...
def is_image_file(file_path) -> bool:
    try:
        print(f"Attempting to open image at: {file_path}")  # Debug print
        with PILImage.open(file_path) as img:
            img.verify()
    except (OSError, SyntaxError) as e:
        print(f"Failed to verify image: {e}")  # Debug print
        return False
    return True
...
def get_file_paths(files: list[str]):
    ...
    file_paths = []
    for file in files:
        # Replace backslashes with forward slashes before splitting
        file = file.replace('\\', '/')
